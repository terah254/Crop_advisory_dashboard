...
# ── Model path using absolute path so it works on Railway ──
...
FEATURE_COLS = ["temp_max", "temp_min", "precipitation_sum",
...
async def analyze(
    ...
    try:
        ...
        if not daily_list:
            raise HTTPException(
                status_code=500,
                detail="No forecast data returned from WeatherAI API."
            )

        # 3. Engineer features
        df = engineer_features(daily_list)

        # 4. Run ML risk model on each day
        ...
        # 5. Analyze farm image if provided
        tree_analysis = None
